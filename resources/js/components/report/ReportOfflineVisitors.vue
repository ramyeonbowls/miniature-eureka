...
export default {
    ...
    mounted() {
        ...
        table = $('#data_rst').DataTable({
            ...
            pageLength: 25,
            processing: true,
            ajax: "/report/offline-visitors-rpt?nodata=yes",
            columns: [
                ...
                { data: "visitor", class: "text-center text-nowrap" }
            ],
            language: {
                lengthMenu: "_MENU_",
                search: "_INPUT_",
    ...
    methods: {
        ...
        openExecute(){
            ...
            if(!check){
                ...
                table.ajax.url("/report/offline-visitors-rpt?menufn="+ this.$route.name +"&"+ urlParam ).load();
            }
        },
        ...
        openXLS(){
            ...
            if(!check){
                ...
                window.axios({
                    url: '/report/offline-visitors-xls',
                    method: 'POST',
                    responseType: 'blob',
                    data: {
                        PROVINSI: this.filter.provinsi,
                ...
                .then((response) => {
                    ...
                    link.href = url;
                    link.setAttribute('download', 'Laporan Pengunjung Offline ' + this.filter.date + '.xlsx');
