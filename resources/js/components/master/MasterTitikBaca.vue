                                                        ...
                                                        <VeeForm ref="form" v-slot="{ handleSubmit }" as="div">
                                                            <form class="form form-horizontal" @submit.prevent="handleSubmit($event, submit)">
                                                                <div class="form-body">
                                                                    <div class="row">
                                                                        <div class="col-md-4">
                                                                            <label for="name">Titik Baca <span class="text-danger">*</span></label>
                                                                        </div>
                                                                        <div class="col-md-8 form-group">
                                                                            <Field type="text" id="name" v-model="form.field.name" class="form-control" name="name"/>
                                                                                ...
                                                                                <tr v-for="(book, key) in form.field.books" :key="key">
                                                                                    ...
                                                                                    <td>
                                                                                        <Field
                                                                                            ...
                                                                                            class="form-control"
                                                                                            :name="'qty'+key"
                                                                                            :rules="`required|numeric|min_value:1|max_value:${book.mqty}`"
                                                                                        />
                                ...
                                <tr v-for="(buku, i) in paginatedBooks" :key="i">
                                    <td class="text-left">
                                        <div class="custom-control custom-checkbox custom-control-lg">
                                            ...
                                            <input type="checkbox" class="custom-control-input" style="cursor: pointer;" :id="'buku_'+ i" :name="'buku_'+ i" :value="buku.book_id+'|'+buku.title+'|'+buku.sellprice+'|'+buku.copy+'|'+buku.mcopy" v-model="selectedBooks">
                                        </div>
                                    </td>
                                    <td class="text-left">
    ...
    methods: {
        ...
        addCheckboxes() {
            this.selectedBooks.forEach(book => {
                let [book_id, title, sellprice, copy, mcopy] = book.split('|')
        ...
        edit() {
            if (this.selected.length > 0) {
                ...
                let code = this.selected[0].id

                this.form.field.name = this.selected[0].name

                window.axios.get('/master/titik-baca-mst/'+ code +'?menufn='+ this.$route.name)
                .then((response) => {
                    loader.hide();
            ...
            if(this.selected.length > 0) {
                this.$swal({
                    ...
                    if (result.value) {
                        ...
                        window.axios.delete('/master/titik-baca-mst/'+ code +'?menufn='+ this.$route.name)
                            .then(response => {
                                loader.hide();
                                this.selected = [];
                                table.ajax.reload();
                ...
                this.$refs.form.validate().then(result => {
                    if(result.valid) {
                        ...
                        if(this.form.new) {
                            ...
                            window.axios.post('/master/titik-baca-mst?menu_fn='+ this.$route.name, form_data)
                                .then((response) => {
                                    loader.hide();
                                    this.cancel();
                            ...
                            window.axios.post('/master/titik-baca-mst/'+ this.selected[0].id +'?menu_fn='+ this.$route.name, form_data)
                                .then((response) => {
                                    loader.hide();
                                    this.cancel();
        ...
        showBooks() {
            ...
            let loader = this.$loading.show()
            window.axios.post('/getOpt', { 'opt': 'BookMaps'})
            .then((response) => {
                loader.hide()
