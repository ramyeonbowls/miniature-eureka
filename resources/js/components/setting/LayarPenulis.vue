                                                            ...
                                                            <form class="form form-horizontal" @submit.prevent="handleSubmit($event, submit)">
                                                                <div class="form-body">
                                                                    <div class="row">
                                                                        ...
                                                                        <div class="col-md-10 form-group">
                                                                            <Field name="file">
                                                                                <input type="file" ref="layarpenulis_files" id="file" name="file" @change="onChangeIcon" accept=".png,.jpg,.jpeg" />
                                                                            </Field>
...
export default {
    ...
    mounted() {
        ...
        table = $('#data_rst').DataTable({
            ...
            rowId: 'extn',
            order: [[0, "asc"]],
            ajax: "/setting/layar-penulis-mst",
            columns: [
                { data: "id" },
                { data: "title" },
                { data: "flag_aktif" },
    ...
    methods: {
        ...
        clearForm() {
            ...
            this.$refs.layarpenulis_files.value = '';
            this.quill.setText('');
        },

        async onChangeIcon(e) {
            this.form.field.file = this.$refs.layarpenulis_files.files[0];
        },
            ...
            if(this.selected.length > 0) {
                this.$swal({
                    ...
                    if (result.value) {
                        ...
                        window.axios.delete('/setting/layar-penulis-mst/'+ this.selected[0].id +'?menufn='+ this.$route.name)
                            .then(response => {
                                loader.hide();
                                this.selected = [];
                                table.ajax.reload();
                ...
                this.$refs.form.validate().then(result => {
                    if(result.valid) {
                        ...
                        if(this.form.new) {
                            ...
                            window.axios.post('/setting/layar-penulis-mst?menu_fn='+ this.$route.name, form_data)
                                .then((response) => {
                                    loader.hide();
                                    this.cancel();
                            ...
                            window.axios.post('/setting/layar-penulis-mst/'+ this.form.field.id +'?menu_fn='+ this.$route.name, form_data)
                                .then((response) => {
                                    loader.hide();
                                    this.cancel();
