...
class TajukUtamaMasterRepository 
{
	/**
     * @param array $filter
     * @return Collection
    ...
    public function update(object $data, string $id, $client_id): bool
    {
        return DB::transaction(function () use ($data, $id, $client_id) {
            return DB::table('tfitur')->where('id', $id)->where('client_id', '=', $client_id)->where('category', '=', 'TU')
    ...
    public function delete(string $id, $client_id)
    {
        return DB::transaction(function () use ($id, $client_id) {
            $TajukUtama = DB::table('tfitur')
                ->where('client_id', '=', $client_id)
                ->where('category', '=', 'TU')
                ->where('id', $id);

            $TajukUtama_file = $TajukUtama->first();
            @unlink(storage_path('app/public/images/news/'.$TajukUtama_file->file));

            return $TajukUtama->delete();
        });
    }
}