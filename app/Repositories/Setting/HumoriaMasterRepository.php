...
class HumoriaMasterRepository 
{
    ...
    public function update(object $data, string $id, $client_id): bool
    {
        return DB::transaction(function () use ($data, $id, $client_id) {
            return DB::table('tfitur')->where('id', $id)->where('client_id', '=', $client_id)->where('category', '=', 'HU')
    ...
    public function delete(string $id, $client_id)
    {
        return DB::transaction(function () use ($id, $client_id) {
            $Humoria = DB::table('tfitur')
                ->where('client_id', '=', $client_id)
                ->where('category', '=', 'HU')
                ->where('id', $id);

            $Humoria_file = $Humoria->first();
            @unlink(storage_path('app/public/images/news/'.$Humoria_file->file));

            return $Humoria->delete();
        });
    }
}