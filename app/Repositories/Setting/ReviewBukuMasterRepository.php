...
class ReviewBukuMasterRepository 
{
	/**
     * @param array $filter
     * @return Collection
    ...
    public function update(object $data, string $id, $client_id): bool
    {
        return DB::transaction(function () use ($data, $id, $client_id) {
            return DB::table('tfitur')->where('id', $id)->where('client_id', '=', $client_id)->where('category', '=', 'RB')
    ...
    public function delete(string $id, $client_id)
    {
        return DB::transaction(function () use ($id, $client_id) {
            $ReviewBuku = DB::table('tfitur')
                ->where('client_id', '=', $client_id)
                ->where('category', '=', 'RB')
                ->where('id', $id);

            $ReviewBuku_file = $ReviewBuku->first();
            @unlink(storage_path('app/public/images/news/'.$ReviewBuku_file->file));

            return $ReviewBuku->delete();
        });
    }
}