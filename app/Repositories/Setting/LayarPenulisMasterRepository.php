...
class LayarPenulisMasterRepository 
{
	/**
     * @param array $filter
     * @return Collection
    ...
    public function update(object $data, string $id, $client_id): bool
    {
        return DB::transaction(function () use ($data, $id, $client_id) {
            return DB::table('tfitur')->where('id', $id)->where('client_id', '=', $client_id)->where('category', '=', 'LP')
    ...
    public function delete(string $id, $client_id)
    {
        return DB::transaction(function () use ($id, $client_id) {
            $LayarPenulis = DB::table('tfitur')
                ->where('client_id', '=', $client_id)
                ->where('category', '=', 'LP')
                ->where('id', $id);

            $LayarPenulis_file = $LayarPenulis->first();
            @unlink(storage_path('app/public/images/news/'.$LayarPenulis_file->file));

            return $LayarPenulis->delete();
        });
    }
}